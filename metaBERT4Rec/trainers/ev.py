...
def evaluate(scores, labels, k):
    ...
    predictions = (-scores).argsort()
    for predict in predictions:
        for i, _k in enumerate(ks):
            precision, recall, ndcg = _compute_precision_recall(targets, predict, _k)
    ...
    metrics['NDCG@1'] = np.mean(ndcgs[0])
    metrics['NDCG@5'] = np.mean(ndcgs[1])
    metrics['NDCG@10'] = np.mean(ndcgs[2])
    metrics['Recall@1'] = np.mean(recalls[0])
    metrics['Recall@5'] = np.mean(recalls[1])
    metrics['Recall@10'] = np.mean(recalls[2])
    return metrics#precisions, recalls, mean_aps, ndcg)
...
def evaluate_ranking(model, test, train=None, k=10):
    ...
    metrics['Recall@5'] = np.mean(recalls[1])
    metrics['Recall@10'] = np.mean(recalls[2])
    return metrics#precisions, recalls, mean_aps, ndcgs
